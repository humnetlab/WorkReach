...
```{python}
def fit_all_models(city_data, city_name, optimized=False):
    models_dict = {}
...
for city_name, city_data in all_city_data.items():
    ...
    all_models[city_name] = fit_all_models(city_data, city_name)
```
...
for i, city in enumerate(cities_list):
    ...
    for j, model in enumerate(model_types):
        ...
        if i == 0:
            ax.set_title(model_display, fontsize=26)

        if j == 0:
            ax.set_ylabel(f"{city}\nPredicted flows", fontsize=24)
            ax.text(-0.28, 1.04, f"{chr(97 + i)})", transform=ax.transAxes,
                    fontsize=30, fontweight="bold", va="bottom", ha="left")

        if i == len(cities_list) - 1:
            ax.set_xlabel("Observed flows", fontsize=24)
...
```{python}
utility_params_table = create_utility_model_parameters_table(
    all_models["Mexico City"]["Utility"][0], "Mexico City")
display(utility_params_table)
```

```{python}
plot_utility_heatmaps(all_models["Mexico City"]["Utility"][0], distance_range=(0.55,0.6), informality_range=(0.4, 0.6), eci_range=(0.6,1))
# plt.savefig('../figs/substitution_rates_cdmx.pdf', dpi=300,
#             facecolor='white', backend="pdf", edgecolor='none')

# plt.savefig('../figs/substitution_rates_cdmx.png', dpi=300, bbox_inches='tight',
...
for city, data in all_city_data.items():
    Dlog = data["distance_log"]
    ECI = data["eci"]
    INF = data["informality"]
    
    bd, be, bf, tau, k = all_models[city]["Utility"][0][:5]
    
    E_d, E_e, E_f = elasticity_matrices(Dlog, ECI, INF, bd, be, bf, tau, k)
    
    mask = ~np.eye(E_d.shape[0], dtype=bool)
    e_d_vals = E_d[mask & ~np.isnan(E_d)]
    e_e_vals = E_e[mask & ~np.isnan(E_e)]
    e_f_vals = E_f[mask & ~np.isnan(E_f)]
    
    ci_d_lo, ci_d_hi = bootstrap_ci(e_d_vals)
    ci_e_lo, ci_e_hi = bootstrap_ci(e_e_vals)
    ci_f_lo, ci_f_hi = bootstrap_ci(e_f_vals)
    
    row = {
        "City": city,
        "mean ε_distance": e_d_vals.mean(),
        "CI_low ε_distance": ci_d_lo,
        "CI_high ε_distance": ci_d_hi,
        "median ε_distance": np.median(e_d_vals),
        "mean ε_ECI": e_e_vals.mean(),
        "CI_low ε_ECI": ci_e_lo,
        "CI_high ε_ECI": ci_e_hi,
        "median ε_ECI": np.median(e_e_vals),
        "mean ε_informal": e_f_vals.mean(),
        "CI_low ε_informal": ci_f_lo,
        "CI_high ε_informal": ci_f_hi,
        "median ε_informal": np.median(e_f_vals),
    }
    results.append(row)

elasticity_tbl = (pd.DataFrame(results)
                  .set_index("City")
                  .sort_index()
                  .round(3))
...
summary_results = []
for _, row in elasticity_tbl.iterrows():
    city = row.name
    summary_row = {
        "City": city,
        "ε_distance": f"{row['mean ε_distance']:.3f} [{row['CI_low ε_distance']:.3f}, {row['CI_high ε_distance']:.3f}]",
        "ε_ECI": f"{row['mean ε_ECI']:.3f} [{row['CI_low ε_ECI']:.3f}, {row['CI_high ε_ECI']:.3f}]",
        "ε_informal": f"{row['mean ε_informal']:.3f} [{row['CI_low ε_informal']:.3f}, {row['CI_high ε_informal']:.3f}]"
    }
    summary_results.append(summary_row)

summary_tbl = pd.DataFrame(summary_results).set_index("City")
...
Use z-score to compare the two accessibility types

```{python}
z_df = (
    combined_accessibility_df.query("Model == 'Utility'")
    .assign(
...
for col in ["z_dist", "z_surp"]:
    z_df[col] = z_df.groupby("City")[col].transform(winsorise)
...
z_df["diff"] = z_df["PC1"].transform(winsorise)
z_df["z_mean"] = ((z_df["z_dist"] + z_df["z_surp"]) / 2).transform(winsorise)
```
...
accessibility_fig = create_accessibility_boxplots(combined_accessibility_df, city_order, z_df)
# accessibility_fig.savefig('../figs/accessibility_boxplots_sharey.pdf', dpi=300,
#                           facecolor='white', edgecolor='none')
# accessibility_fig.savefig('../figs/accessibility_boxplots_sharey.png', dpi=300,
#                           facecolor='white', edgecolor='none')
plt.show()
```
...
```{python}
maps_fig = create_accessibility_maps(
...
## Creating the plot layout of the graphical abstract

The following code only creates a figure layout from previously created plots.
The original files are created in [this notebook](workreach_fig/workreach_abstract_fig.qmd), except for the substitution rates that is created in a former cell of this notebook.
The reason for this is that they don't need data from the models and I wanted to keep this notebook uncluttered.
...
try:
    ...
    # fig.savefig("../figs/four_panel_layout_workreach_font.png", dpi=300, bbox_inches="tight",
    #             facecolor='white', edgecolor='none')
